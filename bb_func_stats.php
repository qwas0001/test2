...
$statsTop=' . ';
for($i=0;$i<$tKey;$i++) $statsTop.=($i<>$top?'<a href="'.$main_url.'/'.$indexphp.'action=stats&amp;top='.$i.'&amp;days='.$days.'&amp;lst='.$lst.'"'.$nof.' class="mnblnk">'.$l_stats_top.' '.fTopa($i).'</a> . ':$l_stats_top.' '.fTopa($i).' . ');
$makeLim=fTopa($top);
...
if(isset($is_mobile) and $is_mobile) $insbr=$brtag; else $insbr=' / ';

for($i=0;$i<=$lstLim;$i++){
if($i!=$lst and $statsOptL[$i]!='') $statsOpt.=$insbr.'<a href="'.$main_url.'/'.$indexphp.'action=stats&amp;top='.$top.'&amp;days='.$days.'&amp;lst='.$i.'"'.$nof.' class="mnblnk">'.$statsOptL[$i].'</a>';
elseif($statsOptL[$i]!='') $statsOpt.= $insbr.'<strong>'. $statsOptL[$i].'</strong>';
}
...
if($stats_barWidth>$stats_barWidthLim) $key='<a href="'.gen_vthread_url($cols[2], $forumNames[$cols[2]][0], $cols[0], $cols[1], PAGE1_OFFSET+1).'"'.$nof.' class="mnblnk">'.$cols[1].'</a>';
else{
$key2='<a href="'.gen_vthread_url($cols[2], $forumNames[$cols[2]][0], $cols[0], $cols[1], PAGE1_OFFSET+1).'"'.$nof.' class="mnblnk">'.$cols[1].'</a>';
$key='<a href="'.gen_vthread_url($cols[2], $forumNames[$cols[2]][0], $cols[0], $cols[1], PAGE1_OFFSET+1).'"'.$nof.' class="mnblnk">...</a>';
}
...
if(!isset($vMax)) $vMax=$val;
if ($vMax!=0) $stats_barWidth=round(100*($val/$vMax));
if($stats_barWidth>$stats_barWidthLim) $key='<a href="'.$main_url.'/'.$indexphp.'action=userinfo&amp;user='.$cols[0].'"'.$nof.' class="mnblnk">'.$cols[1].'</a>';
else{
$key2='<a href="'.$main_url.'/'.$indexphp.'action=userinfo&amp;user='.$cols[0].'"'.$nof.' class="mnblnk">'.$cols[1].'</a>';
$key='<a href="'.$main_url.'/'.$indexphp.'action=userinfo&amp;user='.$cols[0].'"'.$nof.' class="mnblnk">...</a>';
}
...
if($stats_barWidth>$stats_barWidthLim) $key='<a href="'.gen_vthread_url($cols[3], $forumNames[$cols[3]][0], $cols[0], $cols[2], PAGE1_OFFSET+1).'"'.$nof.' class="mnblnk">'.$cols[2].'</a>';
else{
$key2='<a href="'.gen_vthread_url($cols[3], $forumNames[$cols[3]][0], $cols[0], $cols[2], PAGE1_OFFSET+1).'"'.$nof.' class="mnblnk">'.$cols[2].'</a>';
$key='<a href="'.gen_vthread_url($cols[3], $forumNames[$cols[3]][0], $cols[0], $cols[2], PAGE1_OFFSET+1).'"'.$nof.' class="mnblnk">...</a>';
}
...
if($stats_barWidth>$stats_barWidthLim) $key='<a href="'.$fUrl.'"'.$nof.' class="mnblnk">'.$fName.'</a>';
else{
$key2='<a href="'.$fUrl.'"'.$nof.' class="mnblnk">'.$fName.'</a>';
$key='<a href="'.$fUrl.'"'.$nof.' class="mnblnk">...</a>';
}
$val=parseStatsNum($forumAmnt);
$forumIconTd="<td><a href=\"{$fUrl}\"><img src=\"{$main_url}/img/forum_icons/{$forumNames[$forumId][1]}\" alt=\"{$fName}\" title=\"{$fName}\" class=\"mnblnk\" /></a></td>";
...
$numTopics=parseStatsNum($numTopics)." / <a href=\"{$archives_url}\" class=\"mnblnk\">{$l_archivedTP}</a>: ".parseStatsNum($arcTopics)." / {$l_totalTP}: ".parseStatsNum($totalTopics);
$numPosts=parseStatsNum($numPosts)." / <a href=\"{$archives_url}\" class=\"mnblnk\">{$l_archivedTP}</a>: ".parseStatsNum($arcPosts)." / {$l_totalTP}: ".parseStatsNum($totalPosts);
}
else{
$totalTopics=0;
$totalPosts=0;
