...
if(($user_id==0 and $directEmailGuests) or $user_id>0) $usEmail='<a href="'.$indexphp.'action=senddirect&amp;user='.$user.'" class="mnblnk">'.$l_sendDirect.'</a>';
}
...
else return "<a href=\"{$GLOBALS['forums_url']}/{$GLOBALS['indexphp']}action=userinfo&amp;user={$GLOBALS['user']}\" class=\"mnblnk\">{$val}</a>";
}
}
...
if ($GLOBALS['row'][3]!=1) return $GLOBALS['usEmail']; elseif($GLOBALS['user_id']>0) return '<a href="mailto:'.$val.'" class="mnblnk">'.$val.'</a>'; else return '';
}
}

if(!function_exists('parseUserInfo_user_website')){
function parseUserInfo_user_website($val){
if ($val!='' and $GLOBALS['user_id']>0) return '<a href="'.$val.'" target="_blank"'.$GLOBALS['nof'].' class="mnblnk">'.wrapText($GLOBALS['post_word_maxlength'], $val).'</a>';
else return $val;
}
}
...
$newVal="{$GLOBALS['l_totalTP']}: ".parseStatsNum($coreVal)." / {$GLOBALS['l_liveTP']}: ".parseStatsNum($liveVal)." / <a href=\"{$GLOBALS['main_url']}/{$GLOBALS['indexphp']}action=archives&amp;user={$GLOBALS['user']}\" rel=\"nofollow\" class=\"mnblnk\">{$GLOBALS['l_archivedTP']}</a>: ".parseStatsNum($arcVal);
}
}
else{
...
$newVal="{$GLOBALS['l_totalTP']}: ".parseStatsNum($val)." / {$GLOBALS['l_liveTP']}: ".parseStatsNum($liveVal)." / <a href=\"{$GLOBALS['main_url']}/{$GLOBALS['indexphp']}action=archives&amp;user={$GLOBALS['user']}\" rel=\"nofollow\" class=\"mnblnk\">{$GLOBALS['l_archivedTP']}</a>: ".parseStatsNum($arcVal);
}
}
else{
...
if($i!=1 and !isset($customProfileList) and $whatValue!='') { $what=$l_usrInfo[$i]; $USERINFO.=ParseTpl($usrCell); }
...
$whatValue.="<li><a href=\"{$urlp}\" class=\"mnblnk\"{$nof}>{$lastT[2]}</a></li>";
}
...
$topicVals[$lastT[0]]="<li><a href=\"{$main_url}/{$indexphp}action=search&amp;loc=1&amp;forum={$lastT[1]}&amp;topic={$lastT[0]}&amp;page={$postsAll[$lastT[0]]}\" class=\"mnblnk\" rel=\"nofollow\">{$lastT[2]}</a></li>";
}
while ($lastT=db_simpleSelect(1));
}
...
if($stats_barWidth>$stats_barWidthLim) $key="<a href=\"{$urlp}\" class=\"mnblnk\" {$nof}>{$forumNames[$k]}</a>";
else{
$key2="<a href=\"{$urlp}\" class=\"mnblnk\" {$nof}>{$forumNames[$k]}</a>";
$key="<a href=\"{$urlp}\" class=\"mnblnk\" {$nof}>...</a>";
}
...
$what=$l_member; $whatValue="{$mes1} [<a href=\"{$main_url}/{$indexphp}action=userinfo&amp;user={$user}&amp;activity={$actnew}\" class=\"mnblnk\">{$mes2}</a>]";
$USERINFO.=ParseTpl($usrCell);

/* edit profile link */

$what=$l_editPrefs;
$whatValue="<a href=\"{$main_url}/{$indexphp}action=prefs&amp;adminUser={$user}\" class=\"mnblnk\">&gt;&gt;&gt;</a>";
$USERINFO.=ParseTpl($usrCell);
}
...
/* Custom title */
$title1=$title;
$title=$l_about.' &laquo;'.$row[1].'&raquo;';
