...
Latest File Update: 2017-Mar-14
*/
...
do $modsForums[$row[1]]="<a href=\"{$main_url}/{$indexphp}action=userinfo&amp;user={$row[1]}\" class=\"mnblnk\">{$row[0]}</a>"; while ($row=db_simpleSelect(1));
}
}
unset($xtr);
}
...
if($moderatorsList!='') $moderatorsList=$brtag.'<span class="txtSm">'.$l_moderatorsAre.': '.substr($moderatorsList,0,strlen($moderatorsList)-2).'</span>';
}
