...
if($numReplies>0) $LinkToLastPostInTopic='<a href="'.$lmurl1.'" class="mnblnk">'.$lastPostIcon.'</a>';
else $LinkToLastPostInTopic='';

$topicIcon="<img src=\"{$main_url}/img/topic_supersticky.gif\" class=\"forumIcon\" alt=\"{$topic_title}\" title=\"{$topic_title}\" />";
...
if($numReplies>0 and isset($colst[11]) and $colst[11]!='') $lastPoster=$lastPosterIcon.$colst[11];
elseif($numReplies>0 and isset($pValst[$topic][1])) $lastPoster=$lastPosterIcon.$pValst[$topic][1];
...
$lptxt=($textLd==1?$pValst[$topic][3]:strip_tags(str_replace($brtag, ' ', $pValst[$topic][3])));
$lastPostText=$lptxt;
}
