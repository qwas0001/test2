...
Latest File Update: 2017-Jun-14
*/
...
$deleteProfileLnk="{$l_sepr}&nbsp;<a href=\"{$main_url}/{$indexphp}action=removeuser&amp;user={$adminUser}\" target=\"_blank\" class=\"mnblnk\">{$l_removeUser}</a>";
}
else $adminUser=0;
...
$l_passOnceAgain.=$brtag.'<span class="txtSm">('.$l_onlyIfChangePwd.')</span>';
$l_sub_pass.=$brtag.'<span class="txtSm">('.$l_onlyIfChangePwd.')</span>';
$actionName='editprefs';

if ($userData=db_simpleSelect(0,$Tu,'*',$dbUserId,'=',$user_id)) {

$profileLink="{$brtag}<a href=\"{$main_url}/{$indexphp}action=userinfo&amp;user={$user_id}\" class=\"mnblnk\">{$l_about}</a>";
...
$warning.=$l_emailChangeCode.$brtag;
